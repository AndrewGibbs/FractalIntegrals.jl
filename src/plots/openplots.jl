...
function Plots.plot(
    basis::QuasiUniformBasis{<:LebesgueMeasure{2}},
...
function Plots.plot(
    density::Projection{<:QuasiUniformBasis{<:LebesgueMeasure{2}}};
    plot_type = :none,
    linewidth = 0.0,
    kwargs...,
)
    vals = get_polygon_proj_vals(density, plot_type)
    return Plots.plot(density.basis, vals; linewidth = linewidth, kwargs...)
end
