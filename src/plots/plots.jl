...
function plothelper(
    ...
    if markersize <= 0
        markersize =
            marker_adjust_scale *
            max((markersize_const / length(pts))^(1 / Γ.d) * diam(Γ), MIN_MARKER_SIZE)
    end
...
function plothelper(
    μ::AbstractInvariantMeasure{N,<:Any,<:Any,<:Any},
    max_num_pts,
    markersize,
    markersize_const,
    marker_adjust_scale,
    ...
    if markersize <= 0
        markersize =
            marker_adjust_scale *
            max((markersize_const / length(pts))^(1 / μ.supp.d) * diam(μ), MIN_MARKER_SIZE)
    end
...
function Plots.plot(
    ...
    xpts, ypts, markersize =
        plothelper(Γ, max_num_pts, markersize, markersize_const, marker_adjust_scale)
...
function Plots.plot(
    ...
    xpts, ypts, markersize, weights =
        plothelper(μ, max_num_pts, markersize, markersize_const, marker_adjust_scale)
