...
function singlelayer_operator_laplace(
    μ::AbstractInvariantMeasure{N,<:Any,R,<:AbstractAttractor};
    ambient_dimension::Integer = N,
) where {N,R<:Real}
    ambient_dimension = check_ambient_dimension(ambient_dimension)

    if ambient_dimension == 2
        K = SeparableIntegralOperator(
            μ, #fractal domain
...
function dom2codom_singlelayer_operator_laplace(
    μ₁::M1,
    μ₂::M2;
    ambient_dimension = max(get_ambient_dimension.(μ₁, μ₂)...),
    varargs...,
) where {M1<:AbstractInvariantMeasure,M2<:AbstractInvariantMeasure}
    T = eltype(eltype(μ₁))
    if μ₁ == μ₂
...
function singlelayer_operator_laplace(
    μple::Tuple{Vararg{AbstractInvariantMeasure}};
    ...
    op_block = [
        dom2codom_singlelayer_operator_laplace(
            μ₁,
            μ₂;
...
function singlelayer_operator_laplace(Γ_tuple::Tuple{Vararg{AbstractAttractor}}; varargs...)
    return singlelayer_operator_laplace(HausdorffMeasure.(Γ_tuple); varargs...)
end

## --------------------- single layer opeartor Helmholtz -------------------- ##

function singlelayer_operator_helmholtz(
    μ::AbstractInvariantMeasure{N,<:Any,R,<:AbstractAttractor},
    k::Number;
    ambient_dimension::Integer = N,
) where {N,R<:Real}
    ...
    if ambient_dimension == 2
        K = OscillatorySeparableIntegralOperator(
            μ, #fractal domain
...
function dom2codom_singlelayer_operator_helmholtz(
    μ₁::M1,
    μ₂::M2,
    k::Number;
    ambient_dimension = max(get_ambient_dimension.(μ₁, μ₂)...),
...
function singlelayer_operator_helmholtz(
    ...
    # now define the block of operators
    op_block = [
        dom2codom_singlelayer_operator_helmholtz(
            μ₁,
            μ₂,
            k;
            ambient_dimension = ambient_dimension,
...
function singlelayer_operator_helmholtz(
    Γ_tuple::Tuple{Vararg{AbstractAttractor}},
    k::Number;
    varargs...,
)
    return singlelayer_operator_helmholtz(HausdorffMeasure.(Γ_tuple), k; varargs...)
end
