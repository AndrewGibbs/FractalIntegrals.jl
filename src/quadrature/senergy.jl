...
function similar_test_one(
    sₘ::AbstractSimilarity,
    sₙ::AbstractSimilarity,
    sₘ_::AbstractSimilarity,
    sₙ_::AbstractSimilarity,
)
    return isapprox(sₘ.ρ / sₙ.ρ, sₘ_.ρ / sₙ_.ρ; atol = 100 * eps())
end

function similar_test_two(
    sₘ::AbstractSimilarity,
    sₙ::AbstractSimilarity,
    sₘ_::AbstractSimilarity,
    sₙ_::AbstractSimilarity,
    ...
    return isapprox(sₘ.A * T.A * inv(sₙ.A), sₘ_.A * T_.A * inv(sₙ_.A); atol = 100 * eps())
end

function similar_test_three(
    sₘ::AbstractSimilarity,
    sₙ::AbstractSimilarity,
    sₘ_::AbstractSimilarity,
    sₙ_::AbstractSimilarity,
    ...
    return isapprox(
        sₘ.δ .- sₘ_.δ .- sₘ.ρ * sₘ.A * (T.A / sₙ.ρ * inv(sₙ.A) * sₙ.δ .- T.δ),
        .-sₘ_.ρ * sₘ_.A * (T_.A / sₙ_.ρ * inv(sₙ_.A) * sₙ_.δ .- T_.δ);
        atol = 100 * eps(),
    )
end

function check_if_similar(
    Γ::AbstractAttractor,
    ...
    if test_one_pass
        for T in G, T_ in G_
            if similar_test_two(sₘ, sₙ, sₘ_, sₙ_, T, T_) &&
               similar_test_three(sₘ, sₙ, sₘ_, sₙ_, T, T_)
                pass_all = true
            else
                pass_all = false
            end
...
function check_for_similar_integrals(
    Γ::AbstractAttractor,
    ...
    for ∫∫_index = 1:length(X)
        ...
        this_is_X_similar, ρ = check_if_similar(
            Γ,
            ∫∫_indices_higher_level[1],
            mcat,
            ∫∫_indices_higher_level[2],
            mcat_,
            G₁,
            G₂,
        )
...
function check_for_ℓ_singular_integrals(
    Γ::AbstractAttractor,
    m::Vector{<:Integer},
    n::Vector{<:Integer},
)
    ...
    if m == n || m == [0] || n == [0]
        ...
        if m_ℓ_remainder_depth >= 0 && n_ℓ_remainder_depth >= 0
            m_start = convert_vector_index_to_integer_index(
                [
                    m
                    ones(Int64, m_ℓ_remainder_depth)
                ],
                M::Int64,
            )
            m_end = convert_vector_index_to_integer_index(
                [
                    m
                    M * ones(Int64, m_ℓ_remainder_depth)
                ],
                M::Int64,
            )
            m_range = m_start:m_end

            n_start = convert_vector_index_to_integer_index(
                [
                    n
                    ones(Int64, n_ℓ_remainder_depth)
                ],
                M::Int64,
            )
            n_end = convert_vector_index_to_integer_index(
                [
                    n
                    M * ones(Int64, n_ℓ_remainder_depth)
                ],
                M::Int64,
            )
...
function construct_singularity_matrix(
            ...
            if ~f[∫∫_count]
                ...
                if use_strategy_two # subdivide the largest subfractal
                    if ∫∫_indices[1] == [0]
                        ...
                        diam_m = Γ.diam * prod([Γ.ifs[m].ρ for m in ∫∫_indices[1]])
                    end
                    if ∫∫_indices[2] == [0]
                        diam_m_ = Γ.diam
                    else
                        diam_m_ = Γ.diam * prod([Γ.ifs[m].ρ for m in ∫∫_indices[2]])
                    end
                    if diam_m ≈ diam_m_
                        mrange = 1:M
                        m_range = 1:M
                ...
                for m in mrange, m_ in m_range
                    mcat = vcat_(∫∫_indices[1], m)
                    mcat_ = vcat_(∫∫_indices[2], m_)
                    is_S_similar, ρ, similar_index =
                        check_for_similar_integrals(Γ, S, mcat, mcat_, G₁, G₂, fubuni_flag)
                    ...
                    if !is_singular
                        is_R_similar, ρ, similar_index = check_for_similar_integrals(
                            Γ,
                            R,
                            mcat,
                            mcat_,
                    ...
                    if is_similar
                        if is_S_similar
                            ...
                            similar_indices = R[similar_index]
                        end
                        scale_adjust = similar_scaler(
                            ρ,
                            s,
                            similar_indices[1],
                            similar_indices[2],
                    ...
                    elseif is_S_similar # singular, but seen similar
                        ...
                        if s == 0
                            L[∫∫_count] +=
                                ...
                                log(1 / ρ) *
                                prod(pw₁[mcat]) *
                                prod(pw₂[mcat_]) # log constant adjustment
                        end
...
function s_energy(
    ...
    h_quad::Float64 = 0.0,
    N_quad::Int64 = 0,
    quadrule::Tuple{AbstractArray,AbstractArray,AbstractArray} = getdefault_quad_premap(
        μ₁,
        μ₂;
        h_quad = h_quad,
        N_quad = N_quad,
    ...
    A, B, _, R, L =
        construct_singularity_matrix(μ₁, μ₂, s; use_strategy_two = use_strategy_two)
