...
function repassign!(galerkinreps, fromindex, toindex)
    ...
    return galerkinreps[fromindex] = toindex
end
        ...
        for M_ = 2:M
            ...
            for n in canon_range
                for m in canon_range
                    ...
                    galerkinreps[(rep_range[n]-1)*N+rep_range[m]] =
                        (n + n_offset - 1) * Nglob + m + m_offset
                end
            end
        end
    end
    ...
    if ℓ > 1 # equiv: stop recursing when ℓ==1
        ...
        for m = 1:M
            ...
            for n = 1:M
                ...
                if m != n
                    fractaldiagblocks!(
                        @view(galerkinreps[col_range, row_range]),
                        Int64(N / M),
                        M,
                        Nglob,
...
function get_galerkinreps(
    N::Integer,
    sio::AbstractSingularIntegralOperator,
    Vₕ::FractalBasis,
)
