function (φ::PreQuadP0BasisElement)(f::Function)
    return φ.quadrule.weights' * (f.(φ.quadrule.nodes) .* conj.(φ.(φ.quadrule.nodes)))
end
...
function sesquilinearform(
    op::SmoothIntegralOperator,
    ϕ::PreQuadP0BasisElement,
    ψ::PreQuadP0BasisElement,
)
    x, y, w = combine_quadrules(
        ϕ.quadrule.nodes,
        ϕ.quadrule.weights,
        ψ.quadrule.nodes,
        ψ.quadrule.weights,
...
function sesquilinearform(
    sio::AbstractSeparableIntegralOperator,
    ϕ::PreQuadP0BasisElement,
    ψ::PreQuadP0BasisElement,
    ...
    if dist⁻(ϕ.measure.supp, ψ.measure.supp) <= 0 #norm(ϕ.measure.barycentre - ψ.measure.barycentre) <
        ...
        singular_slf, ρ, similar_index = check_for_similar_integrals(
            sio.measure.supp,
            singular_indices,
            Vector(ϕ.vindex),
            Vector(ψ.vindex),
    ...
    if singular_slf
        ...
        I =
            sio.singularconst * singular_integrals[similar_index] * scale_adjust +
...
function getsingularinfo(μ, s, q::QuadStruct)
    A, B, singular_indices, R, log_adjustments = construct_singularity_matrix(μ, μ, s)
