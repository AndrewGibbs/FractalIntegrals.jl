...
function get_boundingball_centre(ifs::AbstractArray{<:AbstractSimilarity})
    ...
    𝐀 = sum(ifs[m].ρA for m = 1:M) / M * IdMat(ndims)
    𝐁 = sum(ifs[m].δ for m = 1:M) / M
    return (IdMat(ndims) - 𝐀) \ 𝐁
end

function get_boundingball_centre(
    ifs::AbstractArray{OneDimensionalSimilarity{T}},
) where {T<:Real}
    min_pt = T(Inf)
    max_pt = -T(Inf)
...
function diam(ifs::AbstractArray{<:AbstractSimilarity})
    ...
    radius = maximum(norm(ball_centre - ifs[m](ball_centre)) / (1 - ifs[m].ρ) for m = 1:M)
    return 2 * radius
end
...
function dist⁺(Γ::AbstractAttractor, x)
    return sqrt(norm(x - get_boundingball_centre(Γ))^2 + (diam(Γ) / 2)^2)
end
function dist⁻(Γ::AbstractAttractor, x)
    return max(norm(x - get_boundingball_centre(Γ)) - diam(Γ) / 2, 0.0)
end

# -------------------- distance from one attractor to another ------------------ #
function dist⁺(Γ::AbstractAttractor, γ::AbstractAttractor)
    return norm(get_boundingball_centre(γ) - get_boundingball_centre(Γ))
end
function dist⁻(Γ::AbstractAttractor, γ::AbstractAttractor)
    return max(dist⁺(Γ, γ) - diam(Γ) / 2 - diam(γ) / 2, 0.0)
end
